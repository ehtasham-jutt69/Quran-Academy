...
// ─── Static Files ────────────────────────────────────────────────────────────
// On Vercel, __dirname is /var/task/api — we need the actual project root.
// We detect this by checking if index.html exists one level up.
const projectRoot = require('fs').existsSync(path.join(__dirname, 'index.html'))
  ? __dirname
  : path.join(__dirname, '..');
